// ownerproof-3407365-1701438984-4c418644035e

// const textLines = ["1abc2", "pqr3stu8vwx", "a1b2c3d4e5f", "treb7uchet"];
const textLines = [
...
const correctedTextLines = textLines.map((line) => {
  return line
    .replaceAll("one", "o1e")
    .replaceAll("two", "t2o")
    ...
    .replaceAll("six", "s6x")
    .replaceAll("seven", "sev7n")
    .replaceAll("eight", "ei8ht")
    .replaceAll("nine", "n9ne");
});

const calibrationSteps = correctedTextLines.map((line) => {
  let lineDigits = [];
  for (let char of line) {
    if (isFinite(char)) {
      lineDigits.push(char);
    }
  }
  return parseInt(lineDigits[0] + lineDigits[lineDigits.length - 1]);
});

const calibrationSum = calibrationSteps.reduce((a, b) => a + b, 0);
console.log(calibrationSteps, calibrationSum);
